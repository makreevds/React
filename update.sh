...
# Сохраняем БД в stash перед reset
if [ "$DB_SAVED" = true ] && [ -f "$DB_FILE" ]; then
    # Временно переименовываем БД, чтобы git reset её не затронул
    mv "$DB_FILE" "${DB_FILE}.tmp"
fi

if git reset --hard origin/main; then
    echo -e "${GREEN}✓ Код принудительно обновлен до origin/main${NC}"
    
    # Восстанавливаем БД из временного файла или резервной копии
    if [ -f "${DB_FILE}.tmp" ]; then
        echo -e "${YELLOW}🔄 Восстанавливаю базу данных...${NC}"
